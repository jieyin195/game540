...
/**
 * 返回 hand 里所有"最大单张"候选——即其他玩家既无法以同花色更大的单张压过、
 * 也无法以炸弹炸掉的牌。
 * - 主牌炸弹（4张主牌）可以炸任意单张
 * - 副牌炸弹（4张同花色副牌）只能炸同花色的单张
 * hand 里的任意一张牌都可以参选，不限制孤张/成对。
 * @param {Card[]} hand
 * @param {Card[][]} otherHands  其他玩家的手牌数组
 * @param {string|null} trumpSuit
 * @returns {Card[]}
 */
export function getBiggestSingleCandidates(hand, otherHands, trumpSuit) {
    const allOtherCards = otherHands.flat();
    const opponentBombs = otherHands.flatMap(h => getBombs(h, trumpSuit));

    return hand.filter(c => {
        if (allOtherCards.some(o => canBeatSingle(o, c, trumpSuit))) return false;
        const cSuit = getSuitOfCard(c, trumpSuit);
        for (const bomb of opponentBombs) {
            if (isTrump(bomb[0], trumpSuit)) return false; // 主牌炸弹炸任意单张
            if (getSuitOfCard(bomb[0], trumpSuit) === cSuit) return false; // 同花色副牌炸弹
        }
        return true;
    });
}
